...
After minimization, non-zero coefficients $\beta_p$ correspond to influential positions to predict the response. \\
  ...
  The spike and slab method \cite{spikeslab1}\cite{spikeslab2} is also used in the case of linear regression. It is a three steps procedure including a filtering step, a marker effect estimation step and a marker selection step. The filtering and marker effect estimation steps use the spike and slab algorithm whereas the marker selection rely on the elastic net (mixing a $l_1$ penalty whit a $l_2$ penalty). The spike and slab algorithm solves a generalized ridge regression problem ($l_2$ penalty). It is implemented in the R package \texttt{spikeslab} \cite{spikeslabR}. 
...
loss,plot,...)
        ...
        \texttt{pkg} & Either ``HDPenReg'' or ``spikeslab''. Ued package in linear case.\\
