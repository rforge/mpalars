...
@	
				...
				You can now run the normalization taking into account information from the control sample (run with \mbox{\texttt{createArchitecture=FALSE}} because we suppose the architecture was already created during the previous example, if it is not the case you can add the missing parameters to the function and set \mbox{\texttt{createArchitecture=TRUE)}}. 
	...
	\subsection{Accessing the copy-number,  allele B fraction and genotype}		
		...
		\begin{center}
			\begin{tabular}{|l|l|}
				...
				\texttt{normalTumorArray} & Only in the case of normal-tumor study. A csv file or a data.frame\\
				\texttt{file} & The name of the files in dataSetName folder for which\\
	...
	\subsection{From data normalized by aroma}	
		...
		\begin{center}
			\begin{tabular}{|l|l|}
				...
				\texttt{normalTumorArray} & Only in the case of normal-tumor study. A csv file or a data.frame\\
				~~ & containing the mapping between normal and tumor files(Fig. \ref{normalTumorArray}).\\
				\texttt{chromosome} & A vector with the chromosome to be segmented.\\
...
@
	...
	\subsection{For any data provided in matrix}\label{calling}
		...
		\begin{itemize}
			...
			\item \textbf{probdloss} (if you have run \texttt{callingProcess} with \texttt{nclass}=5) A matrix of the same size as \texttt{copynumber} matrix. It contains the probability for each segmented copy-number to be a double loss.
...
@
        ...
        \label{callsegplot}
			...
			The following command executes the code to keep only non normal segments.
<<eval=FALSE>>=
...
\section{SNPs selection}
	...
	Let $X$ the matrix of size $n\times p$ containing the profiles of copy-number (or allele B fraction) for $n$ patients and $y$ a vector of length $n$ containing a response for each profile.
	
The lasso problem \cite{lasso} is to find $\hat{\beta}$ such that 
	\begin{equation}
