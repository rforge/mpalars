...
%	\setkeys{Gin}{width=0.8\textwidth}
	\subsection{Overview}
		This package provides functions to preprocess and analyze genomic data. 
		The package was initially developed to select genomic markers associated with a given phenotype when several samples are available.
		...
		All functions or methods which treat patients individually will be referred as functionalities of a single-patient analysis  while those which gather the information of all the patients will be referred as multi-patients analysis. For both types of analysis, a pre-processing step \ref{sec:norm} is proposed as a wrapper of \texttt{aroma.*} packages \cite{aroma}\cite{affy}. 
		Note that this implies that the pre-processing step is only available for Affymetrix DNA chips. The other functionalities of the package can however be used independently and such procedures are explained in the sections which detail each step of the proposed analysis. The main functionalities proposed for a single-patient analysis are normalization, segmentation, calling and filtering. Multi-patients analysis relies on the use of penalized regularization techniques implemented in glmnet \cite{glmnet}. For the users who would like to go further in the use of penalized regularization techniques, we also provide some guidelines to quickly build inputs for the HDPenReg package and interpret some outputs of this latter one.
		%discussion du papier: HDPenReg toujours en construction



	\subsection{Note on the installation of the package and its dependencies}
	
		The package can be installed either from a GUI (selecting the R-forge repository) or with the command line
<<eval=FALSE>>=
...
@	
		...
		For users who want to perform the single analysis (segmentation and calling), we recommand to install the packages \texttt{changepoint}\cite{pelt} and \texttt{CGHcall} \cite{CGHcall}.
...
@
			...
			Extract these files and replace the following path with the appropriate one pointing to the folder containing the extracted .cdf files (only GenomeWideSNP\_6.cdf and GenomeWideSNP\_6.Full.cdf are needed but you can leave the other files in the same folder):	
...
unzip("./genomewidesnp6_libraryfile.zip",
 files=c("CD_GenomeWideSNP_6_rev3/Full/GenomeWideSNP_6/LibFiles/GenomeWideSNP_6.cdf",
"CD_GenomeWideSNP_6_rev3/Full/GenomeWideSNP_6/LibFiles/GenomeWideSNP_6.Full.cdf"),exdir=".")
...
@						
		...
		\subsubsection{Use of the main functions of the package}	
			\label{mainuse}
        		We suggest to perform single-patient analysis before a multi-patient analysis even if only the pre-processing step, which estimates the copy-number and the allele B fraction signal, is required for the last one.
...
createArchitecture=TRUE, savePlot=TRUE)
@	
	
			The single-patient analysis consists in the segmentation of every copy-number signal of the data. Then, a calling method can be applied on all the segments to label them (e.g. "loss", "normal", "gain"). To perform segmentation and calling at the same time, type the following lines in your R console:
...
@
			...
			To perform a multi-patient analysis, a response is associated with each signal and the goal is to find the most relevant markers in the signal according to the response. You can run the following commands:
...
res=markerSelection("datatest1",dataResponse,chromosome=21:22,signal="CN",onlySNP=TRUE)
@
...
<<eval=FALSE>>=
signalPreProcess(dataSetName="datatest1", chipType="GenomeWideSNP_6",
normalTumorArray=normalTumorArray, createArchitecture=FALSE, savePlot=TRUE)
@		
...
@					
			...
			\paragraph{Selecting steps to be executed to avoid a complete procedure}
				...
				The paper is organized as follows : the section \ref{sec:norm} presents the preprocessing step and how to access normalized copy-number and allele B fraction. If you already have these data, you can skip this section. Then, the single patient analysis is introduced in the next two sections. The segmentation process is described in section \ref{sec:seg}. In section \ref{sec:calling}, we present the calling process of segmented data. The multi-patients analysis is described in section \ref{sec:select}.
...
\section{Data Normalization}
  ...
  |  ...      
		...
		\begin{verbatim}
	createArchitecture(dataSetName,chipType,dataSetPath,chipFilesPath,path,verbose)
		\end{verbatim}
...
<<eval=FALSE>>=
createArchitecture("datatest1","GenomeWideSNP_6",celPATH,chipPATH,".",TRUE)
@
...
<<eval=FALSE>>=
signalPreProcess(dataSetName="datatest1", chipType="GenomeWideSNP_6",
createArchitecture=FALSE,savePlot=TRUE)
@
...
chipFilesPath=chipPATH,path=workdir,createArchitecture=TRUE, savePlot=TRUE)
@
...
\section{Segmentation}
	...
	In the package \texttt{MPAgenomics}, we provide a segmentation method, which uses the PELT method of the cpt.mean function from \texttt{changepoint} package \cite{pelt}.
	...
	\subsection{From data normalized by aroma}	
		...
		\begin{figure}[!h]
			...
			\label{outpelt}
		\end{figure}
		The graphic of the segmented signal is saved in the \texttt{figures/dataSetName/segmentation} folder. A summary of the segmented profile is saved in a BED file (Fig. \ref{outbedpelt}) in the \texttt{segmentation/dataSetName/} folder, it contains 5 columns : chrom, chromStart, chromEnd, probes and means containing the number of the chromosome, the starting position of the segment, the ending position of the segment, the number of probes in the segment and the mean of the segment.\\
...
\section{Calling aberrations in copy-number signal}
	\label{sec:calling}
	Once you have a segmentation of your copy-number signal, you may want to assign a label ($loss$, $normal$ or $gain$) to each segment. This can be done with the \texttt{CGHcall} \texttt{R} package \cite{CGHcall}. A wrapper of the CGHcall calling process is available in \texttt{MPAgenomics}.
	...
	\subsection{From data normalized by aroma}	
		...
		\begin{tabular}{|l|l|}
			...
			\texttt{\dots} & Others parameters for CGHcall function \cite{CGHcall}.\\
			\hline
		\end{tabular}

~~\\

		This function will save every segment in a BED file in the \texttt{segmentation} folder in the working directory and return a data.frame with columns:
...
@
		...
		\begin{figure}[!h]
			...
			\caption{Top: graphics saved in the \texttt{figures} folder. Bottom: output of the function (saved in a BED file too).}
			\label{segcalloutput}
		\end{figure}
...
\section{SNPs selection}
	...
	For the multi-patients analysis of the data-set, the goal is to select some relevant markers according to a response.\\
