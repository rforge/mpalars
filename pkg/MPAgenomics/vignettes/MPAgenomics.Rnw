...
@
    ...
    \subsection{To Go further}
      ...
      \subsubsection{Choice of number of segments in cghseg}
        ...
        Let $k$ the number of segments, the slope heuristic criteria minimizes  
        $$g(k,C)=\frac{1}{n}\left(R(k)+C\times k \times \left(2.5+\log(\frac{n}{k})\right)\right)$$
      
        where $R(k)$ is the residuals between the signal and the segmented signal, and $C$ a constant to optimize.
