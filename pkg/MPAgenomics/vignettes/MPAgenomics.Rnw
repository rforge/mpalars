...
\section{Preliminaries}
  ...
  \texttt{MPAgenomics} wraps and extends fonctionnalities of several packages. Please try to cite the appropriate methodological papers when you use results from the limma software in a publication, as such citations are the main means by which the authors receive professional credit for their work.\\
    The above article describes the TumorBoost normalization method.
    
    If you use TumorBoost normalization in \texttt{MPAgenomics}, please cite:
    \begin{itemize}
      \item \bibentry{TumorBoost}
    \end{itemize}
    The above articles describe the PELT segmentation method.
    
    If you use \texttt{MPAgenomics} to segment copy number signal or allele B fraction signal, please cite:
    \begin{itemize}
      \item \bibentry{pelt}
      \item \bibentry{peltR}
    \end{itemize}
    The above articles describe the CHGcall calling method.
    
    If you use \texttt{MPAgenomics} for calling, please cite:
    \begin{itemize}
      \item \bibentry{CGHcall}
      \item \bibentry{CGHcallR}
    \end{itemize}
	...
	createArchitecture(dataSetName,chipType,dataSetPath,chipFilesPath,path,verbose,tags)
		...
		\begin{center}
			\begin{tabular}{|l|l|}
				...
				\texttt{path} & Path where create the architecture (default=".").\\
				\texttt{verbose} & Print information during the process (default=FALSE).\\
        \texttt{tags} & Appear in the different file names (cdf, ugp, ufl) of the chip. \\
...
@
  ...
  +- rawData/
	...
	\subsection{Accessing the copy-number,  allele B fraction and genotype}		
		...
		~~\\
		The next getter, \texttt{getSymFracBSignal}, returns the symmetrized allele B fraction only for heterozygous positions. To symmetrize the allele B fraction the transformation $x\mapsto 2*|x-0.5|$ is applied. It corresponds to center the data in $0.5$ corresponding to heterozygous allele B fraction then symmetrize and multiply by $2$ to have a signal between $0$ and $1$
...
@
		...
		In our downloaded data-set, we have no control data. We will use the first four samples of our data as control and the 4 others samples as tumor samples. The \texttt{normalTumorArray} files associated is in figure \ref{normalTumorArrayExample}. You can create the \texttt{normalTumorArray} with the following command
...
@	
		...
		\begin{figure}[!h]
			\centering
			\includegraphics[width=0.8\textwidth]{fig/normalTumorArrayExample}\\
			\caption{\texttt{normalTumorArray} file with the first four samples as control for the four other files.}
