...
@	
		...
		For users who want to perform the single analysis (segmentation and calling), we recommend to install the packages \texttt{changepoint}\cite{peltR} and \texttt{CGHcall} \cite{CGHcallR}.
...
\section{Segmentation}
	...
	In the package \texttt{MPAgenomics}, we provide a segmentation method, which uses the PELT method of the \texttt{cpt.mean} function from \texttt{changepoint} package \cite{peltR}.
	...
	\subsection{From data normalized by aroma}	
		...
		The procedure described above can be run by the \texttt{PELTaroma} function if the working directory respects the aroma architecture.
...
@					
	...
	\subsection{For any data provided in matrix}	
		If you have your own normalized data stored in a matrix, you can use the \texttt{PELT} function to perform the PELT segmentation with the parameter choice proposed in \texttt{MPAgenomics}.
