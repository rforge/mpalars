...
%		This output can be used directly in argument of the calling function \texttt{callingProcess} (see section \ref{calling}).
		...
		For example, assume you already have done the preprocessing of your data (see section \ref{norm}). You want to do the segmentation of the copy-number profile of one chromosome for one patient.
...
@
    ...
    \subsection{To Go further}
      ...
      \subsubsection{Choice of the number of segments for cghseg}
        ...
        Let $k$ the number of segments, the criteria \cite{penseg} to minimize is 
        $$g(k,C)=\frac{1}{P}\left(R(k)+C\times k \times \left(2.5+\log(\frac{P}{k})\right)\right) $$
      
        where $R(k)$ is the residuals between the profile and the segmented profile, and $C$ a constant to optimize.
      ...
      \subsubsection{Choice of the number of segments for PELT}
        \label{sec:calib}
        This choice was empirically compared to manual annotations on 70 profiles. It appeared to give better results than the default value included in the original PELT package.
        
        The PELT method is run with a penalty $\rho\times K \times\log(P)$ for a range of $\rho$ with $K$ the number of segments. Then, the evolution of the number of segments is plotted with regards to the $\rho$ parameter (Fig \ref{plateau}). 
...
@
	...
	Figure \ref{SNPoutput} shows the output of the function.
		...
		In this example, two markers are selected: the 1065th and 10046th markers of the profile (markers.index) at positions 20192586 pb and 47227321 pb on the chromosome 22. The coefficient associated with each selected marker are given in coefficient vector.
