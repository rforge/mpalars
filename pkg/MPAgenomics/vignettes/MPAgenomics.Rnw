...
\section{Segmentation}
  ...
  The second segmentation approach uses the PELT method of the \texttt{cpt.mean} function from \texttt{changepoint} package \cite{peltR}.
	...
	\subsection{From data normalized by aroma}	
		...
		\begin{verbatim}	
segmentationAroma(dataSetName,normalTumorArray,chromosome,Rho,listOfFiles,onlySNP=TRUE,
savePlot=TRUE,method="cghseg")
		...
		\begin{center}
			\begin{tabular}{|l|l|}
				...
				\texttt{Rho} & A vector containing all the penalization values to test for the segmentation.\\
		...
		\begin{figure}[!h]
			...
			\caption{Top left: variation of the number of segment with regards to $\rho$. Top right: the frequency of occurrence of breakpoints in all segmentation. Bottom: Segmentation with the optimal $\rho$ found.}
			\label{outpelt}
		\end{figure}
...
savePlot=TRUE,verbose=TRUE)
	...
	\subsection{For any data provided in matrix}	
		...
		\begin{verbatim}	
			segmentation(signal,Rho,position=NULL,plot=TRUE,verbose=TRUE)
		\end{verbatim}
...
@
    ...
    \subsection{To Go further}
      \label{sec:togo}
        In this part, we describe the different calibration methods associated with PELT and cghseg in \texttt{MPAgenomics}. We also show that calibrate the parameter according to a measure like variance can be misleading.
        
        \subsubsection{Calibration of penalty in PELT}
          \label{sec:calib}
          The PELT method is run with a penalty $\rho\times \log(n)$ for a range of $\rho$. Then, the evolution of the number of segments is plotted with regards to the $\rho$ parameter (Fig \ref{plateau}). 
      ...
      \subsubsection{Sample-specific versus common parameter}
        ...
        The \textit{sample-specific choice of $\rho$} in Section~\ref{sec:calib} is compared with a \textit{common choice of  $\rho$} depending on the signal-to-noise ratio within each group of profiles.

        Profiles from a real dataset \cite{citerealdataset} are clustered into groups with homogeneous signal-to-noise ratio (SNR) by using a Gaussian mixture model. Three groups are provided by the BIC criterion. 

        Figure~\ref{fig:classifplateau} displays results (chromosome~1) for each profile (patient). Following Section~\ref{sec:calib} the widest range of $\rho$ values is plotted for each profile. 
...
\section{Calling aberrations in copy-number profiles}
	...
	Starting with your signal and the associated segmented signal, the method assumes that the segmented values follow a mixture of Gaussians. The parameters of the model are estimated with an EM algorithm with a specific initialization. Then, every segment is called by the most likely label according to the model. 
...
After minimization, non-zero coefficients $\beta_p$ correspond to influential positions to predict the response. \\
	...
	To solve this problem, we use the LARS algorithm \cite{lars} that solves the lasso problem for all the values of $\lambda$. In order to choose the best solution, we use a cross validation to select the best values of $\lambda$ and return the associated solution.\\
...
@
	...
	Figure \ref{SNPoutput} shows the output of the function.
		...
		In this example, two markers are selected : the 1065th and 10046th markers of the signal (markers.index) at positions 20192586 pb and 47227321 pb on the chromosome 22. The coefficient associated with each selected marker are given in coefficient vector.
