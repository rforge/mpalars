...
void testModel(Array& data, Law const& law)
{
  ...
  model.printParameters(stk_cout);
  stk_cout << "lnLikelihood =" << model.lnLikelihood() << _T("\n");
  stk_cout << "nbFreeParameters =" << model.nbFreeParameters() << _T("\n\n");
}
...
void testCloneModel(Array const& data, Model& model)
{
  ...
  stk_cout << _T("Estimated parameters=\n");
  p_model->printParameters(stk_cout);
  delete p_model;
  stk_cout << "lnLikelihood =" << model.lnLikelihood() << _T("\n");
  stk_cout << "nbFreeParameters =" << model.nbFreeParameters() << _T("\n\n");
}
...
int main(int argc, char *argv[])
{
  ...
  {
    prob[j] = Law::ILawBase::generator.randUnif();
  ...
  {
    mu[j] = Law::ILawBase::generator.randGauss();
    sigma[j] = Law::ILawBase::generator.randExp();
    nLaw.setMu(j, mu[j]);
  ...
  {
    alpha[j] = Law::ILawBase::generator.randExp()*5;
    beta[j] = Law::ILawBase::generator.randExp();
  ...
  testModel< Array2D<Real>, JointGammaModel<Array2D<Real> >, Law::JointGamma<Array2DPoint<Real> > >(gData, gLaw);
  stk_cout << _T("Test with weights\n");
  testWeightedModel< Array2D<Real>, JointGammaModel<Array2D<Real> >, Law::JointGamma<Array2DPoint<Real> > >(gData, gLaw, w);

  stk_cout << _T("True alpha and beta=\n") << alpha << beta;
  JointGammaModel<Array2D<Real> > gModel;
