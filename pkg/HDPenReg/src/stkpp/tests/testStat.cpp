...
int main(int argc, char *argv[])
{
  ...
  Real mu  = mean(X), wmu = mean(X, W);
  stk_cout << _T("mean(X) = ") << mu << _T("\n");
  stk_cout << _T("mean(X, W) = ") << wmu << _T("\n");

  stk_cout << _T("var(X) = ") << variance(X) << _T("\n");
  stk_cout << _T("var(X, true) = ") << variance(X, true) << _T("\n");
  stk_cout << _T("var(X, W) = ") << variance(X, W) << _T("\n");
  stk_cout << _T("var(X,W,true) = ") << variance(X, W, true) << _T("\n");

  stk_cout << _T("varWithFixedMean(X, mean) = ") << varianceWithFixedMean(X, mu) << _T("\n");
  stk_cout << _T("varwithFixedMean(X, mean, true) = ") << varianceWithFixedMean(X, mu, true) << _T("\n");
  stk_cout << _T("varwithFixedmean(X, W, wmean) = ") << varianceWithFixedMean(X, W, wmu) << _T("\n");
  stk_cout << _T("varwithFixedmean(X, W, wmean, true) = ") << varianceWithFixedMean(X, W, wmu, true) << _T("\n");
  ...
  Y[0] = Arithmetic<Real>::NA();

  stk_cout << _T("X =\n") << X << _T("\n");
  stk_cout << _T("Y =\n") << Y << _T("\n");
  stk_cout << _T("W =\n") << W << _T("\n");

  Real xmu  = mean(X), wxmu = mean(X, W);
  Real ymu  = mean(Y), wymu = mean(Y, W);
  stk_cout  << _T("\n\n");
