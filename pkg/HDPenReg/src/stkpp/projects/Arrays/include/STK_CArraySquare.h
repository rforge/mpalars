...
typedef CArraySquare<Real, 2, 2, Arrays::by_col_>            CArraySquare22;
typedef CArraySquare<Real, 3, 3, Arrays::by_col_>            CArraySquare33;
...
struct Traits< CArraySquare<Type_, SizeRows_, SizeCols_, Orient_> >
{
  private:
    class Void { };
    ...
    typedef CArrayVector<Type_, UnknownSize, 1, Arrays::by_row_> SubColIndirect;
    typedef CArrayVector<Type_, UnknownSize, 1, Arrays::by_col_> SubColDirect;

    typedef CArraySquare<Type_, SizeRows_, UnknownSize, Arrays::by_row_> FixedRowArrayIndirect;
    typedef CArraySquare<Type_, UnknownSize, SizeCols_, Arrays::by_row_> FixedColArrayDirect;

    typedef CArraySquare<Type_, SizeRows_, UnknownSize, Arrays::by_col_> FixedRowArrayDirect;
    typedef CArraySquare<Type_, UnknownSize, SizeCols_, Arrays::by_col_> FixedColArrayIndirect;
...
//                       >::Result SubArray;
    // Transposed type
    typedef CArraySquare< Type_, SizeCols_, SizeRows_, !Orient_> Transposed;
    // The CAllocator have to have the same structure than the CArray
    typedef CAllocator<Type_, Arrays::square_, SizeRows_, SizeCols_, Orient_> Allocator;
...
class CArraySquare <Type, SizeRows_, SizeRows_, Orient_>
      : public ICArray < CArray<Type, SizeRows_, SizeRows_, Orient_> >
{
  public:
    typedef ICArray < CArray<Type, SizeRows_, SizeRows_, Orient_> > Base;
    typedef ArrayBase < CArray<Type, SizeRows_, SizeRows_, Orient_> > LowBase;
    enum
    {
      structure_ = Arrays::square_,
      orient_    = Orient_,
 ...
 **/
template <typename Type, int SizeRows_, int SizeCols_, bool Orient_>
ostream& operator<<(ostream& s, const CArraySquare<Type, SizeRows_, SizeCols_, Orient_>& V)
{ return out2D(s,V);}
