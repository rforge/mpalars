 ...
 **/
template <class ArrayObservable>
class ILatentModel : public IModelBase, public IRunnerBase
{
  protected:
    /** Constructor with data set
     *  @param data the observable data set
     **/
    ILatentModel(ArrayObservable const& data) : IModelBase(data.sizeRows(), data.sizeCols())
                                              , IRunnerBase()
                                              , p_observableData_(&data)
    {}
    /** Constructor with a pointer on the data set
     *  @param p_data a pointer on the constant data set
     **/
    ILatentModel(ArrayObservable const* p_data) : IRunnerBase()
                                                , p_observableData_(p_data)
    {
