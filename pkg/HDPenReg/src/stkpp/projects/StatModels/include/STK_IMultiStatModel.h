...
class IMultiStatModel : public IModelBase, public IRunnerUnsupervised<Array, WColVector>
{
  ...
  protected:
    ...
    IMultiStatModel( IMultiStatModel const& model)
                   ...
                   , p_param_((model.p_param_) ? model.p_param_->clone() : 0)
    { }

  public:
    /** destructor */
    virtual ~IMultiStatModel() { if (p_param_) delete p_param_->asPtrDerived();}
    ...
    /** @return the pointer on the parameters and release it from the class. */
    inline Parameters* release()
    { if (p_param_)
      { Parameters* p =p_param_->asPtrDerived(); p_param_=0; return p;}
      return p_param_;
    }
