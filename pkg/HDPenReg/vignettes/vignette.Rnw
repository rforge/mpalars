%\VignetteIndexEntry{Using HDPenReg}
...
\section{Lasso \& Fusion}
	...
	\subsection{Fusion}
		...
		\begin{equation}
			\label{fused}
	  		\hat{\beta}=argmin\left\{ \|y-\textbf{X}\beta\|_2^2 \right\} \text{~~subject~to~} \|\beta\|_1\leq t_1 \text{~and~} \sum\limits_{i=2}^p |\beta_i-\beta_{i-1}|\leq t_2		
		\end{equation}
		...
		\begin{equation}
			\label{fusion}
			\hat{\beta}=argmin\left\{ \|y-\textbf{X}\hat{\beta}\|_2^2 \right\} \text{~~subject~to~}  \sum\limits_{i=2}^p |\beta_i-\beta_{i-1}|\leq t_2		
		\end{equation}
		...
		\begin{equation}
			\label{fusionlasso}
			\hat{\theta}=argmin\left\{ \|y-\textbf{Z}\theta\|_2^2 \right\} \text{~~subject~to~}  \|\theta\|_1\leq t_2
		\end{equation}
...
\section{\texttt{HDPenReg} package}
	\SweaveOpts{prefix.string = fig/vignette, eps = FALSE, pdf = TRUE}
	...
	\label{sec:LarsPath}
		...
		\begin{tabular}{ll}
			...
			\textbf{mu}          & Intercept.\\
			\textbf{lambda}      & Vector containing $\|\hat{\beta}\|_1$ at each step.\\
			\textbf{addIndex}	& Vector of length "step" containing the index of the dropped variable at the each step, 0 means no variable has been dropped at this step.\\
			\textbf{dropIndex}	& Vector of length "step" containing the index of the added variable at each step, 0 means no variable has been added at this step.\\
			\textbf{step}        & Number of steps of the algorithm.\\
		...
		\begin{itemize}
			...
			\item \textbf{\texttt{coefficient}} To extract in a vector the value of all estimates at a given step. 
		\end{itemize}
		
	\subsection{\texttt{simul}}
		This function simulates copy-number signals in genomic \uline{but it's not based on any biological assumptions}.\\
...
@
		...
		\begin{tabular}{ll}
			...
			\textbf{index}		& $l_1$ norm ratio at which the mean squared error of prediction will be computed.\\
		...
		\begin{tabular}{ll}
			...
			\textbf{fraction}	& The fraction associated to the littlest error.\\
...
@
		...
		The $L_1$-norm fraction with the littlest MSE is \Sexpr{rescvlars$fraction}.
...
@			
				\caption{$\hat{\beta}$ coefficients at the step 3.}
			\end{center}
		\end{figure}
