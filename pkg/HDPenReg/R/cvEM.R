...
#' @export
#' 
EMcvlasso <- function(X , y, lambda=NULL, nbFolds=10, maxSteps=1000, intercept=TRUE, model="linear", burn=30, threshold=1.e-08, eps=1e-5, epsCG=1e-8)
{
  #check arguments
  if(missing(X))
    stop("X is missing.")
...
#' @export
#' 
EMcvfusedlasso <- function(X, y, lambda1, lambda2, nbFolds=10, maxSteps=1000, burn=50, intercept=TRUE, model="linear", eps=1e-5, eps0=1e-8, epsCG=1e-8)
{
  #check arguments
  if(missing(X))
    stop("X is missing.")
