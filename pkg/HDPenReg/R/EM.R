...
#' 
EMlasso <- function(X, y, lambda, maxSteps=1000, intercept=TRUE, model="linear", burn=50, threshold=1e-8, eps=1e-5, epsCG=1e-8)
{
  #check arguments
  if(missing(X))
    stop("X is missing.")
...
#' 
EMfusedlasso <- function(X, y, lambda1, lambda2, maxSteps=1000, burn=50, intercept=TRUE, model="linear", eps=1e-5, eps0=1e-8, epsCG=1e-8)
{
  #check arguments
  if(missing(X))
    stop("X is missing.")
