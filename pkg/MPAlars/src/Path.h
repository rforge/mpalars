...
namespace MPA
{
  class Path
  {
    public:
      ...
      /**@return last state*/
      inline STK::Array2DVector< std::pair<int,Real> > const& lastState() const {return states_.back().coefficients();}
      /**@return lambda of state i*/
      inline Real const lambda(int i) const {return states_[i].lambda();}
      /**@return coefficient j of state i*/
      inline Real varCoeff(int i,int j) const {return states_[i].varCoeff(j);}
      /**@return index variable j of state i*/
      inline int varIdx(int i, int j) const {return states_[i].varIdx(j);}
      /**@return last coefficients*/
      inline Real lastVarCoeff(int i) const {return states_.back().varCoeff(i);}
       ...
       */
      STK::Array2DVector< std::pair<int,Real> >  coeff(Real lambda) const;
       ...
       */
      void addCoeff(STK::Array2DVector<int> const& indexVariables,STK::Array2DVector<Real> const& coefficients,int idxVarAdd,int idxVarDrop);
       ...
       */
      void addCaseUpdate(Real gamma, STK::Array2DVector<Real> const& w, int addIdxVar);
       ...
       * @param gamma step of the update
       */
      void update(Real gamma, STK::Array2DVector<Real> const& w);
       ...
       */
      void addWithDropCaseUpdate(Real gamma, STK::Array2DVector<Real> const& w, int addIdxVar, int dropIdxVar, int dropIdx);
       ...
       * @param dropIdx index (in the vector of coefficients of the previous step) of the variable to delete
       */
      void dropAfterDropCaseUpdate(Real gamma, STK::Array2DVector<Real> const& w, int dropIdxVar, int dropIdx);
